...
static void gpio_init(void)
{
	...
	ROM_GPIOIntTypeSet(GPIO_PORTQ_BASE, GPIO_PIN_0, GPIO_FALLING_EDGE|GPIO_DISCRETE_INT);
	...
	// Servo Homing signal
	ROM_GPIOPinTypeGPIOInput(GPIO_PORTL_BASE, GPIO_PIN_0);
	MAP_GPIOPadConfigSet(GPIO_PORTL_BASE, GPIO_PIN_0, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU);
	ROM_GPIOPinWrite(GPIO_PORTL_BASE, GPIO_PIN_0, 0);
	ROM_GPIOIntTypeSet(GPIO_PORTL_BASE, GPIO_PIN_0, GPIO_FALLING_EDGE);

	// Servo direction
	ROM_GPIOPinTypeGPIOOutput(GPIO_PORTL_BASE, GPIO_PIN_1);
	ROM_GPIOPinWrite(GPIO_PORTL_BASE, GPIO_PIN_1, GPIO_PIN_1);
	MAP_GPIOPadConfigSet(GPIO_PORTL_BASE, GPIO_PIN_1, GPIO_STRENGTH_12MA, GPIO_PIN_TYPE_OD);

	// Air jet
	ROM_GPIOPinTypeGPIOOutput(GPIO_PORTL_BASE, GPIO_PIN_2);
	ROM_GPIOPinWrite(GPIO_PORTL_BASE, GPIO_PIN_2, 0);
}
...
static void isr_init(void)
{
	...
	extern void motor_wait_handler(void);
	ROM_IntEnable(INT_TIMER2B);
	ROM_TimerIntEnable(TIMER2_BASE, TIMER_TIMB_TIMEOUT);
	TimerIntRegister(TIMER2_BASE, TIMER_B, motor_wait_handler);

	extern void motor_homing_handler(void);
	GPIOIntEnable(GPIO_PORTL_BASE, GPIO_INT_PIN_0);
	GPIOIntRegister(GPIO_PORTL_BASE, motor_homing_handler);

	extern void air_jet_handler(void);
	ROM_IntEnable(INT_TIMER3A);
	ROM_TimerIntEnable(TIMER3_BASE, TIMER_TIMA_TIMEOUT);
	TimerIntRegister(TIMER3_BASE, TIMER_A, air_jet_handler);
...
static void timer_init(void)
{
	...
	TimerPrescaleSet(TIMER2_BASE, TIMER_B, 240-1);
	TimerLoadSet(TIMER2_BASE, TIMER_B, 50000-1);
	...
	TimerPrescaleSet(TIMER3_BASE, TIMER_A, 120-1);
	TimerLoadSet(TIMER3_BASE, TIMER_A, 20000-1);
...
{
	ROM_GPIOPinWrite(GPIO_PORTL_BASE, GPIO_PIN_2, GPIO_PIN_2);
