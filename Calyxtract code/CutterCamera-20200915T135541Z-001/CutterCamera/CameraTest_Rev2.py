...
class MainFrame(wx.Frame):
	...
	def Serial_Process(self):
		ser = serial.Serial('COM13',9600, timeout=0.01);print(bool(ser))#460800
		cnt = 0;
		while not self.quit:
			if self.tx_string != '':
				print(self.tx_string);print('lol1')
				...
				ser.write(str.encode('#'));print('lol2')
				image = ''
				prev = time.time()
				timeout = 0;print(image)
				while len(image) < (320*240*2)+3 and timeout < 500:
					data = ser.read(16384);print(16384);print(data)#16384
					if data != '':
						image += str(data)
					else:
						if self.quit:
...
class ExceptionDialog(wx.Dialog):
	def __init__(self, parent, msg):
		...
		lbl = wx.StaticText(self, label='Please report the above traceback to Contrivation Labs.')
		vsizer.Add(lbl, flag=wx.GROW|wx.ALL, border=10)
...
if __name__ == '__main__':
	...
	try:
		app = BE_App(False)
		app.MainLoop();print(1)
	except:
		err_type, err_value, trace_back = sys.exc_info()
		tb = traceback.format_exception(err_type, err_value, trace_back)
		msg = ''.join(tb)
		exception = msg
		app = BE_AppException(True);print(2)
		app.MainLoop()
