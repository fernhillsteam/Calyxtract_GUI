...
doc_desc = 'Threshold Test'
doc_number = ''
doc_rev = 'Rev 1'
#image_file1 = 'capture_chilli_stem.bmp'
image_file1 = 'capture_chilli_stem.bmp'
image_file2 = 'capture_2.bmp'
image_file3 = 'capture - Copy (13).bmp'
...
def algo1(img):
	...
	for i in range(0, len(img), 3):
		...
		if b > 150 and r1 < 180 and r2 < 180:
			pr_img += '\xff'
			pr_img += '\xff'
			pr_img += '\xff'
		else:
	...
	for i in range(240):
		white_sum = 0
		for j in range(320):
			r = ord(tmp[i*320*3+j*3])
			
			if r == 255:
				white_sum += 1
		if white_sum > 20 and white_sum < 30:
			row.append(1)
		else:
			row.append(0)
...
def algo_ratio_detection(img):
	...
	for i in range(0, len(img), 3):
		...
		if ((r > 120 and g < 200) or (g > 220 and b > 220)) and (30 < line < 200):
			pr_img += '\xff'
			pr_img += '\xff'
			pr_img += '\xff'
		else:
...
def algo2(img):
	...
	for i in range(0, len(img), 3):
		...
		#if b & 0x7 or g & 0x3 or r & 0x7:
		
		line = i/3 / 320
		
		if g - b > 5 and g - r > 5 and b < 100 and r < 100 and g > 90 and line > 35 and line < 180:
			pr_img += '\xff'
			pr_img += '\xff'
			pr_img += '\xff'
		else:
	...
	for i in range(240):
		...
		for j in range(320):
			...
			if r == 255:
				white_sum += 1
		if white_sum > 10:# and white_sum < 30:
			row.append(1)
			row_count += 1
		else:
			row.append(0)
	...
	for i in range(0, len(img), 3):
		...
		#if (g - r > 4 and g - b > 4) and (g - r < 15 and g - b < 15) and b < 50 and g > 50 and r < 50:
		if g - b > 5 and g - r > 5 and b < 100 and r < 100 and g > 70 and line > 35 and line < 180:
			pr_img += '\xff'
			pr_img += '\xff'
			pr_img += '\xff'
		else:
...
def algo_original(img):
	...
	for i in range(0, len(img), 3):
		...
		if b < 112  and g > 100 and line > 35 and line < 200:
			pr_img += '\xff'
			pr_img += '\xff'
			pr_img += '\xff'
		else:
...
class MainFrame(wx.Frame):
	def __init__(self, parent, title):
		...
		algo = algo_ratio_detection#algo2#algo_blue_only
		
		self.camera_image1 = wx.Image(image_file1, wx.BITMAP_TYPE_ANY).ConvertToBitmap()
		img = self.camera_image1.ConvertToImage().GetData()
		pr_img = algo(img)
		im = self.convert_rgb_to_bmp(pr_img, image_file1)
		im = StringIO.StringIO(im)
		self.processed_image1 = wx.ImageFromStream(im, type=wx.BITMAP_TYPE_ANY).ConvertToBitmap()
		
		#algo = algo_original
		self.camera_image2 = wx.Image(image_file2, wx.BITMAP_TYPE_ANY).ConvertToBitmap()
		img = self.camera_image2.ConvertToImage().GetData()
		pr_img = algo(img)
		im = self.convert_rgb_to_bmp(pr_img, image_file2)
		im = StringIO.StringIO(im)
		self.processed_image2 = wx.ImageFromStream(im, type=wx.BITMAP_TYPE_ANY).ConvertToBitmap()

		self.camera_image3 = wx.Image(image_file3, wx.BITMAP_TYPE_ANY).ConvertToBitmap()
		img = self.camera_image3.ConvertToImage().GetData()
		pr_img = algo(img)
		im = self.convert_rgb_to_bmp(pr_img, image_file3)
		im = StringIO.StringIO(im)
		self.processed_image3 = wx.ImageFromStream(im, type=wx.BITMAP_TYPE_ANY).ConvertToBitmap()
	...
	def OnPaint(self, event):
		if self.ready:
			...
			dc.DrawBitmap(self.camera_image1, 10, 10)
			dc.DrawBitmap(self.processed_image1, 410, 10)
			
			dc.DrawBitmap(self.camera_image2, 10, 270)
			dc.DrawBitmap(self.processed_image2, 410, 270)

			
			dc.DrawBitmap(self.camera_image3, 10, 530)
			dc.DrawBitmap(self.processed_image3, 410, 530)
	...
	def CreateRegPanel(self, panel):
		...
		szr.Add(lbl, flag=wx.ALIGN_CENTER)
		szr.Add(txt, flag=wx.ALIGN_CENTER)
		btn = wx.Button(pnl, wx.ID_ANY, 'Send')
		szr.Add(btn, flag=wx.ALIGN_CENTER)
		self.Bind(wx.EVT_BUTTON, self.OnSend, btn)
	...
	def convert_rgb_to_bmp(self, image, outfile):
		c = []
		for i in range(0, len(image), 3):
			r = (ord(image[i]) >> 3) & 0xFF
			g = (ord(image[i+1]) >> 2) & 0xFF
			b = (ord(image[i+2]) >> 3) & 0xFF
			tmp = r << 11 | g << 5 | b
			#tmp = tmp >> 8 | tmp << 8
			c.append(tmp)
	...
	def Serial_Process(self):
		...
		while not self.quit:
			if self.tx_string != '':
				...
				prev = time.time()
				timeout = 0
				while len(image) < (320*240*2)+3 and timeout < 10:
					data = ser.read(16384)
					if data != '':
						image += data
					else:
				...
				if timeout < 10:
					...
					image = image[:-3]
					self.ready = False
					im = self.convert_rgb565_to_bmp(image, image_file1)
					im = StringIO.StringIO(im)
					self.camera_image1 = wx.ImageFromStream(im, type=wx.BITMAP_TYPE_ANY).ConvertToBitmap()
					self.camera_image1.SaveFile(image_file1, type=wx.BITMAP_TYPE_BMP)
					self.ready = True
				else:
